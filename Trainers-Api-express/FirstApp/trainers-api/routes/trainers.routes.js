  ...
  try {
    ...
    if (isCurrentlyTeaching === "false") {
      const inactiveTrainers = trainers.filter(
        (trainer) => !trainer.isCurrentlyTeaching
      );
      return res.send(inactiveTrainers);
    } else if (isCurrentlyTeaching === "true") {
      const currentTrainers = trainers.filter(
        (trainer) => trainer.isCurrentlyTeaching
      );
    ...
    if (sortBy === "coursesAsc") {
      const descTrainers = trainers.sort(
        (trainerN1, trainerN2) =>
          trainerN2.coursesFinished - trainerN1.coursesFinished
      );
...
trainersRouter.post("/", async (req, res) => {
  try {
    ...
    if (
      ...
      throw new Error("Invaid trainer data");
