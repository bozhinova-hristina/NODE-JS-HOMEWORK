...
class Trainer {
  constructor(firstName, lastName, email, password, isCurrentlyTeaching, timeEmployed, coursesFinished){
    this.id = uuid();
...
static async registerTrainer(trainerData){
  ...
  const trainers = await this.getAllTrainers();
  const trainerExist  = trainers.some(trainer => trainer.email === trainerData.email);
  ...
  const { firstName, lastName, email, password, isCurrentlyTeaching, timeEmployed, coursesFinished} = trainerData;
...
const { password: trainerPassword, ...trainerWithoutPassword } = foundTrainer;
return trainerWithoutPassword;
